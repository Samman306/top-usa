...
export default function ServicePageClient({ params }: Props) {
  ...
  try {
    ...
    const paramsData = typeof params.value === 'string' ? JSON.parse(params.value) : params.value;
    citySlug = paramsData?.city || '';
    serviceSlug = paramsData?.service || '';
    
    // Log for debugging
    console.log('Params data:', { citySlug, serviceSlug });
  } catch (error) {
    console.error('Error parsing params:', error);
    // Extract from URL if parsing fails
    const pathParts = window.location.pathname.split('/');
    if (pathParts.length >= 4) {
      citySlug = pathParts[3] || '';
      serviceSlug = pathParts[4] || '';
      console.log('Extracted from URL:', { citySlug, serviceSlug });
    }
  }
  
  // Get city and state information with fallback
  const { city, state } = findCityState(citySlug || 'los-angeles');
  
  if (!city || !state) {
    console.error('City or state not found for slug:', citySlug);
    // Use default instead of 404
    const defaultCity = { name: 'Los Angeles', slug: 'los-angeles', zipCode: '90001' };
    const defaultState = { name: 'California', abbreviation: 'CA', slug: 'california' };
    // Continue with defaults instead of showing 404
  }
  
  // Find service with fallback
  const service = allPracticeAreas.find(
    (area) => {
  ...
  if (!service) {
    ...
    // Continue with a default instead of showing 404
  }
      ...
      <section className="bg-gradient-to-r from-primary/90 to-primary py-16 md:py-24 text-primary-foreground">
        <div className="container">
          <div className="flex flex-wrap items-center gap-2 mb-6 text-primary-foreground/80">
            ...
            <Link href="/locations" className="hover:text-primary-fo  reground text-sm inline-flex items-center">
              Locations
            </Link>
            <span className="text-sm">/</span>
            <Link
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center">
            ...
            <div>
              ...
              <div className="space-y-4">
                ...
                <p className="text-gray-300">
                  {service.title} cases in {city?.name} require specialized knowledge of both state and local regulations. Our
                ...
                <p className="text-gray-300">
                  ...
                  {service.title.toLowerCase()} attorneys are ready to provide the skilled advocacy and personalized attention
                  you deserve.
                </p>
                <div className="pt-4">
                  <Link
        ...
        <div className="container">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <div className="bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-700">
              ...
              <p className="text-gray-300">
                We treat each {service.title.toLowerCase()} case in {city?.name} with the individual attention it deserves,
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-700">
              ...
              <p className="text-gray-300 mb-4">
                If you're dealing with a {service.title.toLowerCase()} issue in {city?.name}, don't wait to get the legal help
      ...
      <section className="py-12 md:py-16 bg-yellow-500 text-black">
        <div className="container text-center">
          ...
          <p className="text-black/90 max-w-2xl mx-auto mb-8">
            Our {city?.name} {service.title.toLowerCase()} attorneys are ready to help with your legal needs. Contact us today
