...
export default function ServicePageClient({ city, service, stateInfo }: ServicePageClientProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const matchedService = allPracticeAreas.find((area) => area.slug === serviceSlug)
  ...
  if (!cityData || !serviceData) {
    return (
      <div className="container my-16 text-center py-24 text-red-500 font-medium">
        <h2 className="text-2xl font-bold mb-2">404 - Page Not Found</h2>
        <p>
          Unable to load information for {citySlug ? `"${citySlug}"` : "the city"}{" "}
          {serviceSlug ? `and "${serviceSlug}" service` : ""}.
        </p>
        <div className="mt-6">
        ...
        <div className="container">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-6">
            ...
            {stateInfo.name && (
              <>
                <Link
                  href={`/locations/state/${slugify(stateInfo.slug)}`}
            ...
            <div>
              ...
              <div className="space-y-4">
                <p className="text-gray-300">
                  At TOP USA LAW, we specialize in {(serviceInfo?.title || "legal").toLowerCase()} cases in{" "}
                  {cityInfo?.city || "your city"} and throughout {state?.name || "United States"}. Our attorneys have
                  extensive experience handling these specific legal matters in the local courts.
                </p>
                <p className="text-gray-300">
                  {serviceInfo?.title || "Legal"} cases in {cityInfo?.city || "your city"} require specialized knowledge
                  of both state and local regulations. Our team stays up-to-date with all relevant laws to provide you
                ...
                <p className="text-gray-300">
                  ...
                  {(serviceInfo?.title || "legal").toLowerCase()} attorneys are ready to provide the skilled advocacy
        ...
        <div className="container">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-700">
              ...
              <p className="text-gray-300">
                Our attorneys have deep knowledge of {cityInfo?.city || "your city"}'s local courts, judges, and legal
                procedures specific to {(serviceInfo?.title || "legal").toLowerCase()} cases.
              </p>
            </div>
            <div className="bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-700">
              <h3 className="text-xl font-bold mb-3 text-white">Proven Results</h3>
              <p className="text-gray-300">
                We've successfully handled hundreds of {(serviceInfo.title || "legal").toLowerCase()} cases in{" "}
                {cityInfo.city || "your city"}, securing substantial settlements for our clients.
              </p>
            </div>
            <div className="bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-700">
              <h3 className="text-xl font-bold mb-3 text-white">Personalized Approach</h3>
              <p className="text-gray-300">
                We treat each {(serviceInfo.title || "legal").toLowerCase()} case in {cityInfo.city || "your city"} with
                the individual attention it deserves, tailoring our strategy to your specific situation.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            <div className="bg-gray-800 rounded-lg p-6 shadow-sm border border-gray-700">
              ...
              <p className="text-gray-300 mb-4">
                If you're dealing with a {(serviceInfo.title || "legal").toLowerCase()} issue in{" "}
                {cityInfo.city || "your city"}, don't wait to get the legal help you need. Contact our experienced
      ...
      <section className="py-12 md:py-16 bg-yellow-500 text-black">
        <div className="container text-center">
          ...
          <p className="text-black/90 max-w-2xl mx-auto mb-8">
            Our {cityInfo.city || "experienced"} {(serviceInfo.title || "legal").toLowerCase()} attorneys are ready to
            help with your legal needs. Contact us today to schedule a free consultation at our local office.
          </p>
          <Link
            href="/contact"
      ...
      <BreadcrumbSchema
        items={[
          ...
          { name: "Locations", url: "https://www.topusalaw.com/locations" },
          {
            ...
            url: `https://www.topusalaw.com${getStateUrl(state?.slug || "united-states")}`,
          },
          {
            name: cityInfo?.city || "Your City",
            url: `https://www.topusalaw.com/locations/${cityInfo?.slug || "unknown-city"}`,
          },
          {
            name: serviceInfo?.title || "Legal Services",
            url: `https://www.topusalaw.com${getCityServiceUrl(cityInfo?.slug || "unknown-city", serviceInfo?.slug || "legal-services")}`,
          },
        ]}
      />
