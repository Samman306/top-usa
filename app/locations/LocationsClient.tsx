...
const LocationCard = ({ location }: { location: Location }) => {
  return (
    <Link href={`/locations/cities/${location.slug}`} className="block group">
...
const CitiesView = ({ locations }: { locations: Location[] }) => {
  ...
  const currentLocations = locations.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-yellow-600 mb-6">All Cities</h2>
...
const StatesView = ({ locations }: { locations: Location[] }) => {
  ...
  const stateGroups: Record<string, Location[]> = {}

  // Process locations to create state groups
  locations.forEach((location) => {
    const stateName = location.state_name || "Unknown State"
    if (!stateGroups[stateName]) {
      stateGroups[stateName] = []
    }
    stateGroups[stateName].push(location)
  })

  // Get sorted unique state names
  ...
  const currentStates = states.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold text-yellow-500 mb-6">All States</h2>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {currentStates.map((state) => {
          // Get the pre-grouped locations for this state
          const stateLocations = stateGroups[state]

          // Remove duplicate cities within a state by using city name as key
          const uniqueCities = Array.from(new Map(stateLocations.map((item) => [item.name, item]))).map(
            ([_, item]) => item,
          )

          return (
            <Link key={state} href={`/locations/states/${state.toLowerCase()}`} className="block group">
              <div className="bg-gray-800 rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 border border-gray-700 group-hover:-translate-y-1 transform overflow-hidden">
                {/* Decorative Top Bar */}
                <div className="h-2 bg-gradient-to-r from-yellow-500/20 via-yellow-600/20 to-yellow-700/20"></div>
...
export default function LocationsClient() {
  ...
  // Popular cities and states for quick filtering
  const popularCities = [
    "New York",
    "Los Angeles",
    "Chicago",
  ...
  useEffect(() => {
    const fetchCities = async () => {
      try {
        ...
        // Try to fetch data from the built-in locations API endpoint first
        // This is likely already connected to the correct data source
        console.log("Attempting to fetch locations from the existing API...")
        try {
          const locationsResponse = await fetch("/api/cities")

          if (locationsResponse.ok) {
            const locationsData = await locationsResponse.json()
            console.log("Successfully fetched data from locations API")
        ...
        if (result && result.success && Array.isArray(result.data) && result.data.length > 0) {
          ...
          console.log("First 3 transformed locations:", transformedLocations.slice(0, 3))
