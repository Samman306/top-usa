...
export async function GET(request: NextRequest, { params }: { params: { city: string } }) {
  try {
    ...
    if (!state) {
      return NextResponse.json({ error: "State not found for this city" }, { status: 404 })
    }

    // Get keypoints for this city
    const keypoints = await sheetsService.getKeypoints({
      citySlug: citySlug,
      city: cityData.name,
    })
    ...
    return NextResponse.json({ error: "Failed to process city data" }, { status: 500 })
  }
}
